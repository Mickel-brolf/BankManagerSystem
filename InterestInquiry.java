...
import java.io.*;
import sun.audio.*;
public class InterestInquiry extends JFrame
{
int color;
...
JPanel header;
int accno1;
InterestInquiry(String title,int accno,int v)
{
color=v;
accno1=accno;
...
try
 {
     ...
     if(rs.next())
     {
         display_name=new JLabel("Your Interest Rate is"+"  "+rs.getString("Interest"));
         display_name.setFont(username);
         display_name.setForeground(Color.BLACK);
         display_name.setBounds(90,51,300,40);   
     }
 }
 catch(Exception e)
 {
...
{
InterestInquiry obj=new InterestInquiry("InterestInquiry",0,0);
} 
...
{
InterestInquiry.this.exitaudio();
InterestInquiry.this.hideFrame();
}
if(btnop.getSource()==back)
{
InterestInquiry.this.backaudio();
